...
describe StudentTopicExp, type: :model do
  ...
  describe '#self.current_level' do
    ...
    it 'returns level 3 correctly' do
      student_topic_exp = create_student_topic_exp(student,topic,800)
  ...
  describe '#self.next_level_exp' do
    ...
    it 'the default next level exp is topic level one exp' do
      expect(StudentTopicExp.next_level_exp(nil,topic)).to eq 100
    end

    it 'returns exp needed for next level when next level is level 1' do
      create_student_topic_exp(student,topic,0)
      expect(StudentTopicExp.next_level_exp(student,topic)).to eq 100
    end

    it 'returns exp needed for next level when next level is level 2' do
      create_student_topic_exp(student,topic,100)
      expect(StudentTopicExp.next_level_exp(student,topic)).to eq 200
    end

    it 'returns exp needed for next level when next level is level 3' do
      create_student_topic_exp(student,topic,300)
      expect(StudentTopicExp.next_level_exp(student,topic)).to eq 400
    end
  end
  ...
  describe '#self.current_level_exp' do
    ...
    it 'returns the current level exp needed when eg 1' do
      create_student_topic_exp(student,topic,500)
      expect(StudentTopicExp.current_level_exp(student,topic)).to eq 200
    end

    it 'returns the current level exp needed when eg 2' do
      create_student_topic_exp(student,topic,2400)
      expect(StudentTopicExp.current_level_exp(student,topic)).to eq 900
    end
  end
